...
export class BioCrowdsComponent implements AfterViewInit {
    ...
    goals: THREE.Vector3[] = [new THREE.Vector3(window.innerWidth / 20, window.innerHeight / 20, 0)];
    obstacles: any[] = [];
	paths: THREE.Vector3[][] = [];
	newPath = [];
	infos: Info[] = [];
    ...
    play() {
        ...
        if (this.dirty) {
            ...
            world.dimensions = new THREE.Vector2(200, 200);
    ...
    save() {
        ...
        world.obstacles = this.obstacles;
        world.dimensions = new THREE.Vector3(200, 200, 200);
